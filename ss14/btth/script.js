...
let addToCart = (itemData) => {
    return `
        <li>
            <span class="cart-item-name" id="cart-${itemData.id}">${itemData.name} <span class="quantity" quantity="1">x1</span></span>
            <div>
...
productList.addEventListener("click",(element) => {
    ...
    if(button.classList.contains("btn-add")){
        ...
        let cartElement = cartList.querySelector(`#cart-${elementID}`)
        ...
        if(cartElement){
            ...
            onCart["cart-"+elementID]["quantity"] = quantity;
            updateTotalMoney(Object.values(onCart));
        }else{
            cartList.innerHTML += addToCart(elementData);
            onCart["cart-"+elementID] = {price: elementData.price,quantity: 1},
...
cartList.addEventListener("click",(event) => {
    ...
    if(button.classList.contains("btn-remove")){
        ...
        let key = nameSpan.getAttribute("id");
        delete onCart[key];
        li.remove();

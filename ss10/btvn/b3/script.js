...
let getFilteredTotalGoals = (teamPlayers,minGoals) => {
    let filteredPlayers = teamPlayers.filter(element => {
        return element.goals < minGoals;
    });

    let sumOfGoal = 0;
    for (const key of filteredPlayers) {
       sumOfGoal += key.goals;
    }
    return sumOfGoal;
};

console.log(getFilteredTotalGoals(players,50));
